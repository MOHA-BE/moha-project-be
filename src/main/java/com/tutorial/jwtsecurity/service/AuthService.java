...
public class AuthService {
    ...
    @Transactional
    public UserResponseDto signup(UserrRequestDto userrRequestDto) {
        if (userRepository.existsByUserId(userrRequestDto.getUserId())) {
            throw new RuntimeException("이미 가입되어 있는 유저입니다");
        }

        User user = userrRequestDto.toMember(passwordEncoder);
        return UserResponseDto.of(userRepository.save(user));
    }

    @Transactional
    public TokenDto login(UserrRequestDto userrRequestDto) {
        // 1. Login ID/PW 를 기반으로 AuthenticationToken 생성
        UsernamePasswordAuthenticationToken authenticationToken = userrRequestDto.toAuthentication();
